...
length1 = 8 * 8 * 64
layer2 = rand_fc(h_pool1, length1, [19, 23, 11], [13, 2]);
...
length4 = 24 * 11 
W_sp = W_sparse(length4, 1)
p = permute(layer4, W_sp, length4, 1)

Wlast = weight_variable([17, 1, 1, 10])
blast = bias_variable([10])
mid = tf.nn.conv2d(p, Wlast, strides=[1, 7, 1, 1], padding='SAME') + blast
y_conv=tf.nn.softmax(tf.reduce_max(mid, reduction_indices = [1,2]))
