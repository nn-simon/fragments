...
y_max = tf.argmax(y, 1)
sess.run(tf.initialize_all_variables())
y_label = y_max.eval(feed_dict={y: mnist.train.labels})
for i in range(200000):
  if i%200 == 0:
    yy_pred = y_pred.eval(feed_dict={x: mnist.train.images}) 
    train_accuracy = accuracy_expand(y_label, yy_pred, maps);
    maps = adjust(y_label, 10, yy_pred, length3)
    #train_accuracy = accuracy.eval(feed_dict={
    #    x:batch[0], y_: batch[1]})
    print("step %d, training accuracy %g"%(i, train_accuracy))
    for nnl in range(0, length3):
      print("%d,"%maps[nnl]),
    print("")
  batch = mnist.train.next_batch(50)
