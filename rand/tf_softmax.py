...
h_pool3 = tf.nn.avg_pool(h_conv3, [1, 17, 1, 1], strides=[1, 7, 1, 1], padding='SAME')
...
length3 = 159
fc_in = tf.reshape(h_pool3, [-1, length3])
W_fc = weight_variable([length3, 10])
b_fc = bias_variable([10])
y_conv=tf.nn.softmax(tf.matmul(fc_in, W_fc) + b_fc)
